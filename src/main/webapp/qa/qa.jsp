...
%>
<%
int pageSize = 3;  // 한 페이지에 표시될 게시글 개수
...
try {
    ...
    String countSql = "SELECT COUNT(*) FROM questions WHERE 1=1 ";
            ...
            int paramIndex = 1;
                    ...
                    while (rs.next()) {
                    	 String formattedDate = sdf.format(rs.getTimestamp("created_at"));
                    	  String adminReply = rs.getString("admin_reply"); // 🔹 관리자 답변 가져오기
            %>
         <tr>
         <td>
...
<div style="text-align: center; margin-top: 20px;">
    ...
    for(int i = 1; i <= pageCount; i++) {
        if(i == pageNum) {
            ...
            %>
            <a href="qa.jsp?pageNum=<%= i %><%= keyword != null ? "&keyword=" + keyword : "" %>">[<%= i %>]</a>
            <%
        }
    }
    %>
...
<% } %>
    ...
    <form action="qa.jsp" method="get" class="search-form">
        <input type="text" name="keyword" placeholder="제목을 입력하세요" value="<%= (keyword != null) ? keyword : "" %>" class="search-input">
